            ...
            <div class="col-md-12">
                <div class="input-group">
                    ...
                    <div class="input-group-append">
                        <button class="btn btn-outline-secondary" @click="searchOrder">search</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        var app = new Vue({
            ...
            methods: {
                searchOrder() {
                    var _this = this;
                    if (this.order_id == '') {
                        swal("ผิดพลาด",'กรุณาระบุ order', "error");
                        return false;
                    }
                    axios.get('{{url('/back-office/order')}}'+'/'+this.order_id)
                ...
                submit() {
                    ...
                    axios.post('{{url('/back-office/order')}}'+'/'+this.order_id,{status:this.order.status,sign_img:sign_img,sign_length:sign_length,_method:'PUT'})
                        .then(function(response){
                            if (response.data.status==200) {
                                swal({
                                    icon: "success",
