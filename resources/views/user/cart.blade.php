                ...
                <table class="table table-striped">
                    ...
                    @if(!$carts->isEmpty())
                        @foreach($carts as $cart)
                            <tr>
                                <td width="150">
                                    <img class="img-fluid" src="{{url('/uploads/'.$cart->item->image)}}" style="height: 150px; object-fit: cover;">
                                </td>
                                <td class="align-middle">{{$cart->item->id}}</td>
                                <td class="align-middle">{{$cart->item->title}}</td>
                                <td class="align-middle">
                                    <button type="button" class="btn btn-danger" @click="removeItem({{$cart->item->id}})">ลบ</button>
                                </td>
                            </tr>
                        @endforeach
                    @endif
